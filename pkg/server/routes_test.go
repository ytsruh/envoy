...
type mockEnvironmentVariableHandler struct{}

func (m *mockEnvironmentVariableHandler) CreateEnvironmentVariable(w http.ResponseWriter, r *http.Request) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(http.StatusCreated)
	return json.NewEncoder(w).Encode(map[string]interface{}{"id": 1, "key": "API_KEY", "value": "secret"})
}

func (m *mockEnvironmentVariableHandler) GetEnvironmentVariable(w http.ResponseWriter, r *http.Request) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(http.StatusOK)
	return json.NewEncoder(w).Encode(map[string]interface{}{"id": 1, "key": "API_KEY", "value": "secret"})
}

func (m *mockEnvironmentVariableHandler) ListEnvironmentVariables(w http.ResponseWriter, r *http.Request) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(http.StatusOK)
	return json.NewEncoder(w).Encode([]map[string]interface{}{{"id": 1, "key": "API_KEY", "value": "secret"}})
}

func (m *mockEnvironmentVariableHandler) UpdateEnvironmentVariable(w http.ResponseWriter, r *http.Request) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(http.StatusOK)
	return json.NewEncoder(w).Encode(map[string]interface{}{"id": 1, "key": "API_KEY", "value": "new_value"})
}

func (m *mockEnvironmentVariableHandler) DeleteEnvironmentVariable(w http.ResponseWriter, r *http.Request) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(http.StatusOK)
	return json.NewEncoder(w).Encode(map[string]string{"message": "Environment variable deleted successfully"})
}

func TestRegisterEnvironmentVariableHandlers(t *testing.T) {
	t.Parallel()
	s := &Server{
		router:    echo.New(),
		dbService: &mockDBService{},
	...
	mockHandler := &mockEnvironmentVariableHandler{}
	s.RegisterEnvironmentVariableHandlers(mockHandler, s.jwtSecret)
	...
	tests := []struct {
		...
		{
			name:           "create environment variable route - with auth",
		...
		{
			name:           "get environment variable route - with auth",
			method:         http.MethodGet,
			path:           "/projects/1/environments/1/variables/1",
			headers:        map[string]string{"Authorization": "Bearer valid-token"},
			expectedStatus: http.StatusUnauthorized, // JWT will fail with mock token
		},
		{
			name:           "list environment variables route - with auth",
			method:         http.MethodGet,
			path:           "/projects/1/environments/1/variables",
			headers:        map[string]string{"Authorization": "Bearer valid-token"},
			expectedStatus: http.StatusUnauthorized, // JWT will fail with mock token
		...
		{
			name:           "update environment variable route - with auth",
		...
		{
			name:           "delete environment variable route - with auth",
		...
		{
			name:           "create environment variable route - without auth",
			method:         http.MethodPost,
			path:           "/projects/1/environments/1/variables",
			headers:        map[string]string{},
		...
		{
			name:           "get environment variable route - without auth",
			method:         http.MethodGet,
			path:           "/projects/1/environments/1/variables/1",
			headers:        map[string]string{},
			expectedStatus: http.StatusUnauthorized, // No JWT header
